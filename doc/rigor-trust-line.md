...
- All SSG-5–8 device models are complete: timer, UART, NIC (with DMA coherence), and disk — all with axiom-free test vectors.
...
| G5 | ~~No devices~~ → **partial** | ~~IPI/interrupt delivery, DMA, timers, I/O are outside the model~~ | **Partially closed**: the interrupt controller (SSG-3) and IOMMU/DMA translation safety (SSG-4) are fully modeled and proved — `intc.sail` + `intc_proofs.v` + `intc_priority.v`, `iommu_proofs.v` + `vtd_proofs.v` + `smmu_proofs.v` + `amdvi_proofs.v` + all weak lifts. **Timer (SSG-5) closed (2026-08-22):** timer.sail (HartTimer/TimerDevice), timer_ops.v, timer_proofs.v — per-hart mtime/mtimecmp with monotonic tick, pending bits, set/ack operations, 5 headline theorems + 18 axiom-free test vectors (multi-hart, overflow, persistence, ack-target isolation). **UART (SSG-6) closed (2026-08-22):** 8250/16550 register set with write_thr, tx_complete, read_rbr operations, 15 axiom-free test vectors (flow-control, interrupt readiness, two-char sequence, scratch preservation). **NIC (SSG-7) closed (2026-08-22):** DMA descriptor ring model (DmaDesc + NetRegs types), tx_pending, rx_pending, tx_advance_head, rx_advance_tail, 12 axiom-free test vectors (descriptor wraparound, TX/RX independence). Bug fix: ring index wraparound precedence. **DMA coherence closed (2026-08-22):** `net_dma_coherence.v` proves ring_base_valid, tx/rx_advance_iommu_invariant (4 axiom-free test vectors). **Disk (SSG-8) closed (2026-08-22):** DiskCmd + DiskRegs types, cmd_pending, cmp_pending, cmd_submit, cmp_complete, 20 axiom-free test vectors (flush/barrier identification, cmd/cmp independence, ring-base preservation). Bug fix: ring index wraparound precedence. **SSG-9 grouping hierarchy documented** — `doc/ssg9-grouping-hierarchy.md` scoping multikernel domains, framekernel, and NORMA clusters. |
