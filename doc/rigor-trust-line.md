...
- No data RAM, address decoding, bus, or cache model (memory = the page table).
...
| G3 | Memory = PTE association list | No data RAM, decode, bus, or cache — so load/store correctness is out of reach | A real memory model (address space + data), later increment |
