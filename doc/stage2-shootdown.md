...
**Foundational sources.** The release/acquire + view-transfer idioms the S2.2c/S2.5
...
proof style, not a separately-verified artefact (see `rigor-trust-line.md`). The
operational model the ORC11/iRC11 release/acquire rules are sound against is the
Promising semantics, now on hand as
`~/Dokumente/A_Promising_Semantics_for_Relaxed-Memory_Concurrency.pdf`
(Kang, Hur, Lahav, Vafeiadis, Dreyer — **POPL 2017**). Together GPS + Promising
pin down the two ends of the trust line for S2.2: Promising says what the weak
memory model *is*; GPS says how to *reason* in it; gpfsl is the bridge the proofs
actually use.
