...
Definition paddr : Type := bits 48.

Record LaEntry := {
  LaEntry_vppn : bits 35;
  LaEntry_ps : Z;
  LaEntry_pfn0 : bits 36;
  LaEntry_pfn1 : bits 36;
}.
Arguments LaEntry : clear implicits.
#[export]
Instance Decidable_eq_LaEntry : EqDecision LaEntry.
   intros [x0 x1 x2 x3].
   intros [y0 y1 y2 y3].
...
Defined.
#[export]
Instance Countable_LaEntry : Countable LaEntry.
refine {|
  encode x := encode (LaEntry_vppn x, LaEntry_ps x, LaEntry_pfn0 x, LaEntry_pfn1 x);
  decode x := '(x0, x1, x2, x3) ← decode x;
              mret (Build_LaEntry x0 x1 x2 x3)
|}.
abstract (
  intros [x0 x1 x2 x3];
...
Notation "{[ r 'with' 'LaEntry_vppn' := e ]}" :=
  match r with Build_LaEntry _ (_ as f1) (_ as f2) (_ as f3) =>
    Build_LaEntry e f1 f2 f3 end (at level 0).
Notation "{[ r 'with' 'LaEntry_ps' := e ]}" :=
  match r with Build_LaEntry (_ as f0) _ (_ as f2) (_ as f3) =>
    Build_LaEntry f0 e f2 f3 end (at level 0).
Notation "{[ r 'with' 'LaEntry_pfn0' := e ]}" :=
  match r with Build_LaEntry (_ as f0) (_ as f1) _ (_ as f3) =>
    Build_LaEntry f0 f1 e f3 end (at level 0).
Notation "{[ r 'with' 'LaEntry_pfn1' := e ]}" :=
  match r with Build_LaEntry (_ as f0) (_ as f1) (_ as f2) _ =>
    Build_LaEntry f0 f1 f2 e end (at level 0).
#[export]
Instance dummy_LaEntry : Inhabited (LaEntry) := {
  inhabitant := {|
    LaEntry_vppn := inhabitant;
    LaEntry_ps := inhabitant;
    LaEntry_pfn0 := inhabitant;
    LaEntry_pfn1 := inhabitant
|} }.
