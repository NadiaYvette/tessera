...
Definition undefined_DiskCmd '(tt : unit) : M (DiskCmd) :=
   ...
   returnM (({| DiskCmd_lba := w__0;
                DiskCmd_len := w__1;
                DiskCmd_opcode := w__2;
                DiskCmd_flags := w__3;
                DiskCmd_status := w__4 |})).

Definition undefined_DiskRegs '(tt : unit) : M (DiskRegs) :=
   (undefined_bitvector (48)) >>= fun (w__0 : mword 48) =>
   (undefined_bitvector (16)) >>= fun (w__1 : mword 16) =>
   ...
   returnM (({| DiskRegs_cmd_ring_base := w__0;
                DiskRegs_cmd_ring_len := w__1;
                DiskRegs_cmd_head := w__2;
                DiskRegs_cmd_tail := w__3;
                DiskRegs_cmp_ring_base := w__4;
                DiskRegs_cmp_ring_len := w__5;
                DiskRegs_cmp_head := w__6;
                DiskRegs_cmp_tail := w__7;
                DiskRegs_ctrl := w__8;
                DiskRegs_status := w__9;
                DiskRegs_irq_status := w__10 |})).
