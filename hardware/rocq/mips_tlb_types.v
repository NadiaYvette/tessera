...
Record MipsEntry := {
  MipsEntry_vpn : bits 30;
  MipsEntry_pfn : bits 44;
  MipsEntry_level : Z;
  MipsEntry_esp : bool;
}.
Arguments MipsEntry : clear implicits.
#[export]
Instance Decidable_eq_MipsEntry : EqDecision MipsEntry.
   intros [x0 x1 x2 x3].
   intros [y0 y1 y2 y3].
...
Defined.
#[export]
Instance Countable_MipsEntry : Countable MipsEntry.
refine {|
  encode x := encode (MipsEntry_vpn x, MipsEntry_pfn x, MipsEntry_level x, MipsEntry_esp x);
  decode x := '(x0, x1, x2, x3) ← decode x;
              mret (Build_MipsEntry x0 x1 x2 x3)
|}.
abstract (
  intros [x0 x1 x2 x3];
...
Notation "{[ r 'with' 'MipsEntry_vpn' := e ]}" :=
  match r with Build_MipsEntry _ (_ as f1) (_ as f2) (_ as f3) =>
    Build_MipsEntry e f1 f2 f3 end (at level 0).
Notation "{[ r 'with' 'MipsEntry_pfn' := e ]}" :=
  match r with Build_MipsEntry (_ as f0) _ (_ as f2) (_ as f3) =>
    Build_MipsEntry f0 e f2 f3 end (at level 0).
Notation "{[ r 'with' 'MipsEntry_level' := e ]}" :=
  match r with Build_MipsEntry (_ as f0) (_ as f1) _ (_ as f3) =>
    Build_MipsEntry f0 f1 e f3 end (at level 0).
Notation "{[ r 'with' 'MipsEntry_esp' := e ]}" :=
  match r with Build_MipsEntry (_ as f0) (_ as f1) (_ as f2) _ =>
    Build_MipsEntry f0 f1 f2 e end (at level 0).
#[export]
Instance dummy_MipsEntry : Inhabited (MipsEntry) := {
  inhabitant := {|
    MipsEntry_vpn := inhabitant;
    MipsEntry_pfn := inhabitant;
    MipsEntry_level := inhabitant;
    MipsEntry_esp := inhabitant
|} }.
