...
Record HartTimer := {
  HartTimer_mtimecmp : bits 64;
  HartTimer_pending : bool;
}.
Arguments HartTimer : clear implicits.
#[export]
Instance Decidable_eq_HartTimer : EqDecision HartTimer.
   intros [x0 x1].
   intros [y0 y1].
...
Defined.
#[export]
Instance Countable_HartTimer : Countable HartTimer.
refine {|
  encode x := encode (HartTimer_mtimecmp x, HartTimer_pending x);
  decode x := '(x0, x1) ← decode x;
              mret (Build_HartTimer x0 x1)
|}.
abstract (
  intros [x0 x1];
...
Notation "{[ r 'with' 'HartTimer_mtimecmp' := e ]}" :=
  match r with Build_HartTimer _ (_ as f1) => Build_HartTimer e f1 end (at level 0).
Notation "{[ r 'with' 'HartTimer_pending' := e ]}" :=
  match r with Build_HartTimer (_ as f0) _ => Build_HartTimer f0 e end (at level 0).
#[export]
Instance dummy_HartTimer : Inhabited (HartTimer) := {
  inhabitant := {| HartTimer_mtimecmp := inhabitant; HartTimer_pending := inhabitant
|} }.


Record TimerDevice := {
  TimerDevice_mtime : bits 64;
  TimerDevice_harts : list HartTimer;
}.
Arguments TimerDevice : clear implicits.
#[export]
Instance Decidable_eq_TimerDevice : EqDecision TimerDevice.
   intros [x0 x1].
   intros [y0 y1].
  cmp_record_field x0 y0.
  cmp_record_field x1 y1.
...
Defined.
#[export]
Instance Countable_TimerDevice : Countable TimerDevice.
refine {|
  encode x := encode (TimerDevice_mtime x, TimerDevice_harts x);
  decode x := '(x0, x1) ← decode x;
              mret (Build_TimerDevice x0 x1)
|}.
abstract (
  intros [x0 x1];
  rewrite decode_encode;
...
Notation "{[ r 'with' 'TimerDevice_mtime' := e ]}" :=
  match r with Build_TimerDevice _ (_ as f1) => Build_TimerDevice e f1 end (at level 0).
Notation "{[ r 'with' 'TimerDevice_harts' := e ]}" :=
  match r with Build_TimerDevice (_ as f0) _ => Build_TimerDevice f0 e end (at level 0).
#[export]
Instance dummy_TimerDevice : Inhabited (TimerDevice) := {
  inhabitant := {| TimerDevice_mtime := inhabitant; TimerDevice_harts := inhabitant
|} }.
